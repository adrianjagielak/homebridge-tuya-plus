...
* [+] **Tuya Cloud is now a transparent, global fallback for every device** — add a top-level `cloud` credentials block once and the plugin keeps a single Tuya Cloud/MQTT session alive in the background, falling back to it whenever a device can't be reached on the LAN: a flaky moment, or hardware that's never local (battery-powered "sleepy" irrigation timers and the like). The plugin stays **LAN-first** — local is always tried first and preferred — and cloud remains **strictly opt-in**: nothing runs unless credentials are present.
  * **No per-device setup.** Existing LAN configs (numeric data-points) and cloud configs (string codes) both work over either transport, because the data-point id↔code map is learned from Tuya's device shadow (`/v2.0/cloud/thing/{id}/shadow/properties`). Each accessory tries the LAN first and falls back to the cloud only when the device is unreachable locally; if both fail, HomeKit shows "No Response" as before. This mirrors the official Tuya/Smart Life app (LAN when possible, cloud as a backup).
  ...
  * `"cloud": true` on a device now means *cloud-first* (for a sleepy device that never appears on the LAN — it skips LAN discovery); `"cloud": false` keeps a device strictly local; `"fallback": false` in the cloud block limits the cloud to only the explicitly-cloud devices (the older opt-in-per-device behavior). Legacy per-device `cloud` credential objects are still accepted but deprecated — a single top-level `cloud` block is preferred.
  * The existing **IrrigationSystem** accessory works unchanged, and now gains the same LAN+cloud fallback.
