...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        if (!this.device.context.noRotationSpeed) {
            if (Array.isArray(this.device.context.fanSpeedSteps) && this.device.context.fanSpeedSteps.length) {
                this._rotationSteps = [null];
                this._rotationStops = {0: 0};
                const steps = this.device.context.fanSpeedSteps;
                ...
                steps.forEach((step, idx) => {
                    this._rotationStops[step] = Math.round(((idx + 1) * 100) / count);
                });
            } else {
    ...
    convertRotationSpeedFromTuyaToHomeKit(value) {
        if (Array.isArray(this.device.context.fanSpeedSteps)) {
