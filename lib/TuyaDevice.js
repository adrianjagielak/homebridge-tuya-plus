...
class TuyaDevice extends EventEmitter {
    constructor(props = {}) {
        ...
        // cloudPrimary: the device is known to be unreachable over the LAN (e.g. a
        // sleepy battery device flagged `cloud: true`). It operates off the cloud
        // and never waits for, or warns about, the LAN.
        this.cloudPrimary = !!props.cloudPrimary;

        // LAN backend — created later, once an IP is known (see attachLan), unless
        // the device has no local key (cloud-only).
        this.lan = null;
    ...
    _connect() {
        if (this.cloud) {
            ...
            // Safety net for the LAN-primary-without-map registration guard above.
            if (!this.cloudPrimary && this.context.key) {
                setTimeout(() => {
    ...
    // the device is cloud-only, or once it has the numeric map (state is dual-keyed
    // and so resolves a numeric-DP config), or once the LAN has been given its head
    // start and didn't show up.
    _mayRegisterFrom(which) {
        if (which === 'lan') return true;
        if (this.cloudPrimary || !this.context.key) return true;
